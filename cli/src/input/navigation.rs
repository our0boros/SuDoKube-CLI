...
// ── 跨面方向转换：确保蛇跨面后不回头、不越界 ──
// 基于 move_on_surface 的跨面映射推导，
// 保证蛇跨面后在新面上向面内部走。

/// 蛇跨面时方向转换
/// old_face: 蛇原来所在的面
/// dx, dy: 蛇原来在面局部坐标中的运动方向
/// new_face: 蛇跨面后到达的新面
/// 返回: 蛇在新面上应该沿什么方向继续前进
pub fn convert_face_dir(old_face: Face, dx: i8, dy: i8, _new_face: Face) -> (i8, i8) {
    // 查表法：每种面+方向组合对应一个跨面后的新方向
    // 这些值由 move_on_surface 的跨面行为推导得出，
